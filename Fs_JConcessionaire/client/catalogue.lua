...
function CatalogueMenu()
	local catalogueee = RageUI.CreateMenu("", "INTERACTION")
	local vehiclemenu = RageUI.CreateSubMenu(catalogueee, "", "INTERACTION")
	local vehiclemenuparam = RageUI.CreateSubMenu(vehiclemenu, "", "INTERACTION")
	...
	RageUI.ButtonWithStyle(Fs_conc.listecatevehi[i2].name, nil, {RightLabel =  "→"},true, function(Hovered, Active, Selected)
	if (Selected) then
...
Citizen.CreateThread(function()
	while true do
		...
		if jobdist <= Marker.DrawDistance and Concess.jeveuxmarker then
			...
			if jobdist <= 1.0 then
				Timer = 0
					RageUI.Text({ message = "Appuyer sur ~b~[E]~s~ pour ouvrir le ~b~Catalogue", time_display = 1 })
